                  ...
                  <div class="col-md-12 px-1">
                     <div class="berita__hero">
                      <div class="position-absolute px-3 py-2 text-white" style="background-color: rgba(0,0,0,0.2);border-top-left-radius:8px;z-index:999">
                         <a href="/smartphone?category={{ $posts[0]->category->slug }}" class="text-decoration-none text-white" >
                            {{ $posts[0]->category->name }}
                         </a>
                      </div>
                      <div class="image">
                         <a class="text-decoration-none" href="/postSmartphone/{{ $posts[0]->slug }}">
                      ...
                      <div class="card-body">
                         ...
                         <div class="card-text">
                            <label style="font-size:0.8em;margin-bottom:5px">By :
                                <a class="text-decoration-none" href="/smartphone?author={{ $posts[0]->author->username }}">{{ $posts[0]->author->name }}</a> in <a class="text-decoration-none" href="/smartphone?category={{ $posts[0]->category->slug }}">{{ $posts[0]->category->name }}</a>
                                <span class="px-2">
                         ...
                         <a href="/postSmartphone/{{ $posts[0]->slug }}" class="btn btn-outline-primary icon-link">Read More 
                     ...
                     <div class="berita flex" style="height: auto">
                        <div class="contents">
                           <div class="position-absolute px-3 py-2 text-white" style="background-color: rgba(0,0,0,0.3);border-top-left-radius:8px;z-index:999">
                              <a href="/smartphone?category={{ $post->category->slug }}" class="text-decoration-none text-white">
                                 {{ $post->category->name }}
                              </a>
                           </div>
                           <div class="image">
                              <a class="text-decoration-none" href="/postSmartphone/{{ $post->slug }}">
                           ...
                           <div class="card-body">
                              ...
                              <div class="card-text" style="min-height: 4rem">
                                 <label style="font-size:0.7em;margin-bottom:5px">By : <a class="text-decoration-none" href="/smartphone?author={{ $post->author->username }}">{{ $post->author->name }}</a> in <a class="text-decoration-none" href="/smartphone?category={{ $post->category->slug }}">{{ $post->category->name }}</a></label>
                              </div>
                              <div class="card-text text-end p-2" style="bottom:0;right:0;position:absolute">
